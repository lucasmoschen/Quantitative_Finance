...
\begin{align} 
P(Y \leq y) &= P(Y \leq y | Y = X_1)P(Y = X_1) + P(Y \leq y | Y = - X_2)P(Y = - X_2) \\
            &= (1 - \exp(-\frac{1}{2}y))\cdot \frac{1}{3} + P(X_2 \geq -y)\cdot \frac{2}{3} \\
            &= \frac{1}{3}(1 - \exp(-\frac{1}{2}y)) + \frac{2}{3}(1 - \frac{y}{2})^{-2} 
\end{align}

Porém, temos que nos ater aos suportes dessas distribuições. Se $y < 0$, teremos que a $P(X_1 \leq y) = 0$, logo $P(Y \leq y) =  \frac{2}{3}(1 - \frac{y}{2})^{-2}$. Agora, se $y \geq 0$, temos que $P(X_2 \geq -y) \geq P(X_2 \geq 0) = 1$, logo: 
$$F_Y(y) = \begin{cases}
